def shannon_fano(dict):
    def shannon_fano_rec(pair, dict):
        if (len(pair[0]) > 1): pair[0] = shannon_fano_rec(split(pair[0], dict), dict)
        if (len(pair[1]) > 1): pair[1] = shannon_fano_rec(split(pair[1], dict), dict)
...
print(shannon_fano({'B': 12, 'D': 8, 'A': 24, 'C': 10, 'E': 8}))